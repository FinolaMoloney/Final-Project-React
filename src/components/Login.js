...
function Login() {
  ...
  async function handleSubmit(e) {
    ...
    async function handleupdateInfo(e) {
      ...
      try {
        ...
        const response = await axios.put(`http://34.242.53.203:4000/customers/${user}`, updatedCustomerData, {
          headers: {
  ...
  useEffect(() => {
    async function fetchOrderEmail() {
      try {
        const response = await axios.get(`http://34.242.53.203:4000/customers/${user}`, {
          headers: { Accept: 'application/json' },
        });
  ...
  const handleOrderHistoryCheck = async (data) => {
    try {
      const response = await axios.get('http://34.242.53.203:4000/orders', {
        headers: { Accept: 'application/json' },
      });
