...
function Login() {
  ...
  async function handleSubmit(e) {
    ...
    async function handleupdateInfo(e) {
      ...
      try {
        ...
        const response = await axios.put(`http://localhost:4000/customers/${user}`, updatedCustomerData, {
          headers: {
  ...
  useEffect(() => {
    async function fetchOrderEmail() {
      try {
        const response = await axios.get(`http://localhost:4000/customers/${user}`, {
          headers: { Accept: 'application/json' },
        });
