...
function Login() {
  ...
  async function handleSubmit(e) {
    ...
    async function handleupdateInfo(e) {
      ...
      try {
        ...
        const response = await axios.put(`http://3.250.199.113:4000/customers/${user}`, updatedCustomerData, {
          headers: {
  ...
  useEffect(() => {
    async function fetchOrderEmail() {
      try {
        const response = await axios.get(`http://3.250.199.113:4000/customers/${user}`, {
          headers: { Accept: 'application/json' },
        });
  ...
  const handleOrderHistoryCheck = async (data) => {
    try {
      const response = await axios.get('http://3.250.199.113:4000/orders', {
        headers: { Accept: 'application/json' },
      });
